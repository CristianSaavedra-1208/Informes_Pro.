...
def heal_mapping_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if id_rep_col:
        df_rep = df_valid[df_valid[id_rep_col].notna() & (df_valid[id_rep_col].astype(str).str.strip() != "") & (df_valid[id_rep_col].astype(str).str.strip().str.lower() != "nan")]
        for _, row in df_rep.iterrows():
            rep_map[str(row[cls_col]).strip()] = str(row[id_rep_col]).strip()
            
    if id_nota_col:
        df_nota = df_valid[df_valid[id_nota_col].notna() & (df_valid[id_nota_col].astype(str).str.strip() != "") & (df_valid[id_nota_col].astype(str).str.strip().str.lower() != "nan")]
        for _, row in df_nota.iterrows():
            nota_map[str(row[cls_col]).strip()] = str(row[id_nota_col]).strip()
            
    if cf_col:
        df_cf = df_valid[df_valid[cf_col].notna() & (df_valid[cf_col].astype(str).str.strip() != "") & (df_valid[cf_col].astype(str).str.strip().str.lower() != "nan")]
        for _, row in df_cf.iterrows():
            cf_map[str(row[cls_col]).strip()] = str(row[cf_col]).strip()
            
    # 3. Rellenar los valores nulos o vacíos en el DataFrame
    for idx, row in df_healed.iterrows():
        cls_val = str(row[cls_col]).strip()
        if not cls_val or cls_val.lower() == "nan":
            continue
            
        # Para ID_Reporte
        if id_rep_col:
            val_rep = str(row[id_rep_col]).strip()
            if not val_rep or val_rep.lower() == "nan" or pd.isna(row[id_rep_col]):
                if cls_val in rep_map:
                    df_healed.loc[idx, id_rep_col] = rep_map[cls_val]
                    
        # Para ID_Nota_Asociada
        if id_nota_col:
            val_nota = str(row[id_nota_col]).strip()
            if not val_nota or val_nota.lower() == "nan" or pd.isna(row[id_nota_col]):
                if cls_val in nota_map:
                    df_healed.loc[idx, id_nota_col] = nota_map[cls_val]
                    
        # Para Clasificación Flujo Efectivo
        if cf_col:
            val_cf = str(row[cf_col]).strip()
            if not val_cf or val_cf.lower() == "nan" or pd.isna(row[cf_col]):
                if cls_val in cf_map:
                    df_healed.loc[idx, cf_col] = cf_map[cls_val]
