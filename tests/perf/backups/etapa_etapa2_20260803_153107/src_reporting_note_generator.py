            ...
            def get_grouped_sums(merged_df_obj, group_col):
                ...
                for name, gp in grouped:
                    ...
                    for _, r_val in gp.iterrows():
                        ...
                        bal = abs(float(r_val[saldo_col])) if saldo_col in gp.columns else 0.0
                    ...
                    sum_val = gp[saldo_col].sum() if saldo_col in gp.columns else 0.0
                    sum_ini = gp['saldo_inicial'].sum() if 'saldo_inicial' in gp.columns else 0.0
                    sum_deb = gp['debitos'].sum() if 'debitos' in gp.columns else 0.0
                    sum_cred = gp['creditos'].sum() if 'creditos' in gp.columns else 0.0
                    ...
                    for _, r_val in gp.iterrows():
                        ...
                        acct_details.append({
                            'cuenta_id': acc_id,
                            'nombre': r_val[tb_nombre_col] if tb_nombre_col else '',
                            'val': r_val[saldo_col] * factor if saldo_col in gp.columns else 0.0,
                            'inicial': r_val.get('saldo_inicial', 0.0) * factor,
                            'debitos': r_val.get('debitos', 0.0) * factor,
                            'creditos': r_val.get('creditos', 0.0) * factor,
                            'clasificacion_bal': str(r_val.get('Clasificación balance', '')).strip()
                        })
...
class NoteGenerator:
    ...
    def generate(self,
        ...
        def analyze_columns(ws):
            ...
            # Listar empresas reales en data/empresas/ para correspondencia dinámica
            import os, datetime as _dt
            empresas_dir = os.path.join("data", "empresas")
            real_companies = []
            if os.path.exists(empresas_dir):
                real_companies = [d for d in os.listdir(empresas_dir) if os.path.isdir(os.path.join(empresas_dir, d))]
