...
def load_all_entity_contexts(active_entity, periodo_actual, periodo_comp, map_balance_df, map_pl_df):
    ...
    if is_consolidated:
        ...
        tb_comp_list = []
        for co in companies:
            tb_a = TrialBalanceDB.get_trial_balance(co, periodo_actual)
            if tb_a is not None and not tb_a.empty:
                tb_act_list.append(tb_a)
            tb_c = TrialBalanceDB.get_trial_balance(co, periodo_comp) if periodo_comp != "Ninguno" else None
            if tb_c is not None and not tb_c.empty:
                tb_comp_list.append(tb_c)
    ...
    if selected_id:
        ...
        if run_note:
            ...
            if not os.path.exists(template_nota):
                ...
                # Cargar mapeos maestros desde la plantilla base
                try:
                    ...
                    map_balance_df = pd.read_excel(map_bal_local) if os.path.exists(map_bal_local) else None

                try:
                    map_pl_df = pd.read_excel(template_nota, sheet_name="Mapeo Ctas P&L Cubo")
                except:
                    map_pl_local = os.path.join(empresa_path, "map_pl.xlsx")
                    map_pl_df = pd.read_excel(map_pl_local) if os.path.exists(map_pl_local) else None
                ...
                import importlib
                import src.reporting.note_generator
                importlib.reload(src.reporting.note_generator)
