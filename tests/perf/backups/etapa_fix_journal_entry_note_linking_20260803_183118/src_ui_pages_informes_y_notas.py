...
def load_all_entity_contexts(active_entity, periodo_actual, periodo_comp, map_balance_df, map_pl_df):
    ...
    if is_consolidated:
        ...
        # Aplicar eliminaciones intercompañía para saldos recíprocos entre filiales del grupo
        def apply_intercompany_eliminations(ctx):
            if not ctx: return ctx
            # 1. Eliminación en P&L para rubros intercompañía recíprocos (ej. intereses préstamos empresas relacionadas)
            for rubro, items in ctx.get('pl', {}).items():
                for item_key, item_data in items.items():
                    if any(k in item_key for k in ['empresas relacionadas', 'intercompania', 'intercompany']):
                        item_data['val'] = 0.0
                        item_data['inicial'] = 0.0
                        item_data['debitos'] = 0.0
                        item_data['creditos'] = 0.0
            # 2. Eliminación en Nota1/Nota2 para cuentas relacionadas si aplica
            for sec in ('nota1', 'nota2'):
                for item_key, item_data in ctx.get(sec, {}).items():
                    if any(k in item_key for k in ['entidades relacionadas ic', 'cuentas por pagar relacionadas ic', 'cuentas por cobrar relacionadas ic']):
                        item_data['val'] = 0.0
            return ctx
        ...
        if run_note:
            ...
            if not os.path.exists(template_nota):
                ...
                try:
                    ...
                    # La clave incluye empresa y ambos períodos para invalidar automáticamente
                    # si el usuario cambia de empresa o período entre notas.
